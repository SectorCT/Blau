...
const BY_KEY = ENRICHMENT_MINERALS.reduce((acc, mineral) => {
    acc[mineral.key] = mineral;
    return acc;
}, {});
const BY_ELEMENT = ENRICHMENT_MINERALS.reduce((acc, mineral) => {
    acc[mineral.element.toLowerCase()] = mineral;
    return acc;
}, {});
export const MINERAL_ELEMENTS = new Set(ENRICHMENT_MINERALS.map((mineral) => mineral.element));
export const findMineralByKey = (key) => BY_KEY[key.toLowerCase()];
/** Resolve a mineral by element symbol (Ca, Mg, K, …) or by free-form name (Calcium, Mg2+, …). */
export const findMineralByElement = (element) => BY_ELEMENT[element.trim().toLowerCase()];
export const findMineralByName = (name) => {
    const normalized = name.trim().toLowerCase();
    if (!normalized)
        return undefined;
    if (BY_KEY[normalized])

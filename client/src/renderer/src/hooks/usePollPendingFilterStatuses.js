...
export const isFilterStatusWaiting = (s) => {
    if (s == null || typeof s !== 'string')
        return false;
    const n = s.trim().toLowerCase();
    return n === 'pending' || n === 'generating';
};
const listHasWaitingFilters = (list) => list.some((it) => isFilterStatusWaiting(it.status));
/** Run async tasks with a fixed parallelism cap. Failed tasks resolve with null. */
async function fetchProgressBatched(ids, fetcher, concurrency) {
    const out = [];
    let cursor = 0;
 ...
 */
export function usePollPendingFilterStatuses(items, setItems) {
    ...
    useEffect(() => {
        ...
        const poll = async () => {
            ...
            if (!listHasWaitingFilters(current))
                return;
            inFlight = true;
            try {

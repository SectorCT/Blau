import { collectAtomPositions, collectConnections, filterToPollutantLayerRows } from '@renderer/utils/normalizeFilterStructure';
export const IMPORTED_FILTER_ROUTE_ID = 'imported';
const SESSION_KEY = 'blau:imported-filter-json-v1';
export function writeImportedFilterSession(state) {
    try {
...
export function readImportedFilterSession(location) {
    const st = location.state;
    if (st?.importedFilterJson !== undefined)
        return st;
    try {
...
export function isImportedFilterRouteId(id) {
...
const toStringOrUndefined = (value) => typeof value === 'string' && value.trim().length > 0 ? value : undefined;
const normalizeAtomPositions = (value) => {
    ...
    for (const entry of value) {
        ...
        if (x == null || y == null || z == null || !element)
            continue;
        atoms.push({
...
export function normalizeImportedFilterInfo(payload) {
    ...
    const nestedSummaryMetrics = toRecord(rawFilterInfo.summaryMetrics) ?? toRecord(nestedFilterStructure?.summaryMetrics);
    const filterStructureDraft = {
        ...nestedFilterStructure,
        poreSize: toFiniteNumber(nestedFilterStructure?.poreSize) ?? toFiniteNumber(rawFilterInfo.poreSize),
        layerThickness: toFiniteNumber(nestedFilterStructure?.layerThickness) ?? toFiniteNumber(rawFilterInfo.layerThickness),
        latticeSpacing: toFiniteNumber(nestedFilterStructure?.latticeSpacing) ?? toFiniteNumber(rawFilterInfo.latticeSpacing),
        materialType: toStringOrUndefined(nestedFilterStructure?.materialType) ?? toStringOrUndefined(rawFilterInfo.materialType),
    ...
    const filterStructure = {
        ...
        atomPositions: collectedAtoms.length > 0 ? collectedAtoms : fallbackAtoms && fallbackAtoms.length > 0 ? fallbackAtoms : [],
        connections: collectedConns.length > 0 ? collectedConns : fallbackConns && fallbackConns.length > 0 ? fallbackConns : [],
    };
    const importedLayersMerged = filterToPollutantLayerRows(rawFilterInfo.layers ?? nestedFilterStructure?.layers);
    const importedLayers = importedLayersMerged.length > 0 ? importedLayersMerged : undefined;
    const resultPayload = {
        ...(nestedResultPayload ?? {}),
        bindingEnergy: toFiniteNumber(nestedResultPayload?.bindingEnergy) ?? toFiniteNumber(rawFilterInfo.bindingEnergy),
        removalEfficiency: toFiniteNumber(nestedResultPayload?.removalEfficiency) ?? toFiniteNumber(rawFilterInfo.removalEfficiency),
        pollutant: toStringOrUndefined(nestedResultPayload?.pollutant) ?? toStringOrUndefined(rawFilterInfo.pollutant),
        pollutantSymbol: toStringOrUndefined(nestedResultPayload?.pollutantSymbol) ??
    ...
    const hasCoordinates = Array.isArray(filterStructure.atomPositions) && filterStructure.atomPositions.length > 0;
    const hasBondGraph = Array.isArray(filterStructure.connections) && filterStructure.connections.length > 0;
    const hasAnyMetric = typeof filterStructure.materialType === 'string' ||
    ...
    const hasSummaryMetrics = nestedSummaryMetrics != null &&
...
export function buildFilterDetailsFromImportedJson(payload, _fileName) {
    const parsed = toRecord(payload);
    if (!parsed)
        throw new Error('JSON root must be an object.');

...
let locationsWorker = null;
let workerRequestId = 0;
const workerPending = new Map();
const workerPendingReject = new Map();
const getLocationsWorker = () => {
    if (typeof Worker === 'undefined')
        return null;
    if (!locationsWorker) {
        locationsWorker = new GemstatLocationsWorker();
        locationsWorker.onmessage = (event) => {
...
const readLocationsCacheFromStorage = () => {
    ...
    try {
        ...
        if (!Array.isArray(parsed.locations) || typeof parsed.expiresAt !== 'number')
            return null;
...
const writeLocationsCacheToStorage = (cache) => {
    if (typeof window === 'undefined')
        return;
    try {
        window.localStorage.setItem(LOCATIONS_CACHE_KEY, JSON.stringify({
...
export const hasGemstatLocationsCache = () => {
...
const normalizeLocationsSync = (results = []) => {
    const locations = [];
    ...
    for (const item of results ?? []) {
        ...
        if (seenLocationIds.has(normalizedLocationId))
            continue;
        seenLocationIds.add(normalizedLocationId);
        locations.push({
            locationId: normalizedLocationId,
            measurementId: item.measurementId,
            localStationNumber: item.sampleLocation?.local_station_number ?? item.sampleLocation?.station_id ?? null,
...
const normalizeLocations = async (results = []) => {
    ...
    try {
        const locations = await Promise.race([
            request,
            new Promise((_, reject) => window.setTimeout(() => reject(new Error('Map worker timeout')), 10_000)),
        ]);
        return locations;
    }
    catch {
...
const normalizeStationMeasurements = async (locationId, payload) => {
    const worker = getStationMeasurementsWorker();
    const normalizeFallback = () => {
        const rows = normalizeStationRows(payload);
        return {
            measurementId: typeof payload.measurementId === 'string' ? payload.measurementId : `station-${locationId}`,
            locationId: typeof payload.locationId === 'string' ? payload.locationId : locationId,
            stationName: (typeof payload.name === 'string' && payload.name) ||
...
export const getGemstatLocations = async () => {
    if (!locationsCache) {
    ...
    let normalizeDurationMs = null;
    const startedAtIso = new Date().toISOString();
    console.info(`[Map API] locations start: ${startedAtIso}`);
    locationsInFlight = makeAuthenticatedReq({
        method: 'GET',
    ...
    try {
        ...
        console.info(`[Map API] locations end: ${endedAtIso} (request ${requestMsLabel}, normalize ${normalizeMsLabel}, total ${totalMs}ms)`);
        locationsInFlight = null;
    }
};
export const getGemstatStationMeasurements = async (locationId) => {
...
export const getGemstatSnapshot = async (locationId, date // ISO-8601 string per contract
) => {
    return makeAuthenticatedReq({
        method: 'GET',
        path: `/api/measurements/locations/${locationId}/`,
        ...
        parseResponse: async (response) => {
            const payload = (await response.json());
            const targetRow = (payload.rows ?? []).find((row) => row.dateKey === date) ??
...
const toStationRows = (row) => {
    const sourceParams = row.parameters ?? [];
    return sourceParams.map((parameter) => makeStationRowFromParameter({
...
const normalizeStationRows = (payload) => {
    ...
    return measurements
        ...
        return {
            ...
            volume: null,
            temperature: typeof record.temperature === 'number' && Number.isFinite(record.temperature)
                ? record.temperature
                : null,
            ph: typeof record.ph === 'number' && Number.isFinite(record.ph) ? record.ph : null,
            parameterCount: typeof record.parameterCount === 'number' && Number.isFinite(record.parameterCount)

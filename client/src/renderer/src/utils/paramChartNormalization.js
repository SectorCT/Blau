...
function massMicrogramsPerLiter(value, unitRaw) {
...
function isConductivityUnit(unitRaw) {
    const u = canonicalUnit(unitRaw);
    if (!u)
        return false;
    if (u.includes('s/cm') || u.includes('s/m'))
        return true;
    if (u === 'ec' || u === 'conductivity')
        return true;
    return false;
}
function classify(p) {
    ...
    if (ug != null)
        return { ...p, massUgL: ug, kind: 'mass', unitKey: 'mass' };
    if (isConductivityUnit(p.unit))
        return { ...p, massUgL: null, kind: 'conductivity', unitKey: 'conductivity' };
...
export function buildExperimentParameterCharts(params) {
    if (params.length === 0) {

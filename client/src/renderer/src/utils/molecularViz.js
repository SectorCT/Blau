...
import { getAggregateRemovalEfficiencyPercent, getFilterLayers, getSummaryMetrics } from '@renderer/utils/normalizeFilterStructure';
...
function drawGenericMolecule(ctx, node) {
...
export function buildMoleculeDefinitions(info) {
    const params = info.experimentPayload?.params ?? [];
    const layers = getFilterLayers(info);
    const defaultRemovalPct = clamp(getAggregateRemovalEfficiencyPercent(info) ??
        info.resultPayload?.removalEfficiency ??
        getSummaryMetrics(info)?.removalEfficiency ??
        (layers[0]?.removalEfficiency ?? 90), 0, 100);
    ...
    for (const p of params) {
        ...
        if (NON_MOLECULAR_CODES.has(code))
            continue;
...
export class MolecularScene {
    ...
    rebuildNodes() {
        ...
        for (const molecule of this.definitions) {
            const baseCount = molecule.filterable
                ...
                : 90;
            const eff = molecule.filterable && typeof molecule.removalRate === 'number'
    ...
    draw(ctx) {
        ...
        for (const n of this.nodes) {
            if (n.molecule.code === 'NO3')
                drawNO3(ctx, n);
            else if (n.molecule.code === 'PO4')
                drawPO4(ctx, n);
            else if (n.molecule.code === 'H2O')
                drawH2O(ctx, n);
            else if (n.molecule.code === 'FE' || n.molecule.code === 'MN')
                drawMetal(ctx, n);
            else
                drawGenericMolecule(ctx, n);
        }
    }
    drawMembrane(ctx) {

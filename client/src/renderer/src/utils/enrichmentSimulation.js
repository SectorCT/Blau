import { parseTargetBand } from '@renderer/data/enrichmentMinerals';
...
export function buildEnrichmentConfigFromFilterInfo(info) {
    const vm = buildFilterInfoViewModel(info ?? null);
    const minerals = vm.enrichmentMinerals.map((view) => toRuntime(view));
    return {
        minerals,
...
function toRuntime(view) {
    ...
    const targetSteadyMgPerL = bandSpan > 0
    ...
    const mgPerParticle = expectedParticlesAt100 > 0 ? Math.max(targetSteadyMgPerL / expectedParticlesAt100, 1e-6) : 1;
    return {
...
export class EnrichmentEngine {
    ...
    tick(dtFrames, nowMs) {
        ...
        if (this.boundSites.length > 0) {
            ...
            for (const site of this.boundSites) {
                if (nowMs < site.cooldownUntil)
                    continue;
                const m = minerals.get(site.mineralKey);
                if (!m || m.releaseRate <= 0)
                    continue;
    ...
    recomputeConcentrations() {
        ...
        for (const m of this.config.minerals) {
            ...
            if (mgPerL >= m.targetMin && mgPerL <= m.targetMax)
                inRange++;
        }
